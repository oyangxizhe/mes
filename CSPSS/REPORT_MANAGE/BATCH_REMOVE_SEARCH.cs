...
{
    public partial class BATCH_REMOVE_SEARCH : Form
    {
        ...
        private void BATCH_REMOVE_SEARCH_Load(object sender, EventArgs e)
        {
            hint.Text = "";
            ...
            DataTable dtx = bc.getdt("SELECT DISTINCT(BATCHID) FROM BATCH_REMOVE_MST");
            ...
            dtx = bc.getdt("SELECT DISTINCT(NEW_BATCHID) FROM BATCH_REMOVE_MST");
            ...
            if (dtx.Rows.Count > 0)
            {
                foreach (DataRow dr1 in dtx.Rows)
                {
                    ...
                    string suggestWord = dr1["NEW_BATCHID"].ToString();
                    comboBox2.Items.Add(dr1["NEW_BATCHID"].ToString());
        ...
        private void dgvStateControl()
        {
            ...
            this.dataGridView1.MergeColumnNames.Add("母批号");
            //this.dataGridView1.MergeColumnNames.Add("母批号数量");
            //this.dataGridView1.MergeColumnNames.Add("现母批号数量");
            this.dataGridView1.MergeColumnNames.Add("子批号");
            //this.dataGridView1.MergeColumnNames.Add("子批号数量");
            //this.dataGridView1.MergeColumnNames.Add("现子批号数量");
            ...
            dataGridView1.Columns["母批号"].Width = 200;
            dataGridView1.Columns["母批号数量"].Width = 80;
            dataGridView1.Columns["现母批号数量"].Width = 90;
            dataGridView1.Columns["子批号"].Width = 200;
            dataGridView1.Columns["子批号数量"].Width = 80;
            dataGridView1.Columns["现子批号数量"].Width = 90;
        ...
        public void search()
        {
            ...
            sqb.AppendFormat(cBATCH_REMOVE .sqlt);
            sqb.AppendFormat(" WHERE A.BATCHID LIKE '%{0}%' AND A.NEW_BATCHID LIKE '%{1}%'", comboBox1.Text, comboBox2.Text);
...
");
 ...
 (SELECT USER_GROUP FROM USERINFO WHERE USID='" + LOGIN.USID + "'))" );
            ...
            dt = cBATCH_REMOVE.RETURN_HAVE_ID_DT(dt);
            if (dt.Rows.Count > 0)
            {
        ...
        private void btnToExcel_Click(object sender, EventArgs e)
        {
            if (dt.Rows.Count > 0)
            {
                ...
                bc.dgvtoExcel(dataGridView1, "拆批履历");

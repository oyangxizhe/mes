...
{
    public partial class RESUME_SEARCH : Form
    {
        ...
        CRESUME cresume = new CRESUME();
        public RESUME_SEARCH()
        {
            InitializeComponent();
        }
    
        private void RESUME_SEARCH_Load(object sender, EventArgs e)
        {
            hint.Text = "";
...
FROM POSTING_MST  A  ");
        ...
        public void search()
        {
            ...
            sqb.AppendFormat(cresume.sql);
            sqb.AppendFormat(" WHERE A.WOID LIKE '%{0}%' AND G.CO_WAREID LIKE '%{1}%'", comboBox1.Text, comboBox2.Text);
            sqb.AppendFormat(" AND G.WNAME LIKE '%{0}%' AND A.BATCHID LIKE '%{1}%'", textBox1.Text, textBox2.Text);
            sqb.AppendFormat(" AND E.STEP_ID LIKE '%{0}%' AND E.STEP LIKE '%{1}%'", bc.REMOVE_NAME(comboBox3.Text, '-'), textBox3.Text);
 ...
 (SELECT USER_GROUP FROM USERINFO WHERE USID='" + LOGIN.USID + "'))" );
            ...
            dt = cresume.RETURN_HAVE_ID_DT(dt);
            if (dt.Rows.Count > 0)
            {
        ...
        private void btnToExcel_Click(object sender, EventArgs e)
        {
            if (dt.Rows.Count > 0)
            {
                ...
                bc.dgvtoExcel(dataGridView1, "生产履历");

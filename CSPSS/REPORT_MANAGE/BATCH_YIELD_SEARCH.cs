...
{
    public partial class BATCH_YIELD_SEARCH : Form
    {
        DataTable dt = new DataTable();
        basec bc=new basec ();
        CBATCH_YIELD cbatch_yield = new CBATCH_YIELD();
        private string _IDO;
        public string IDO
        {
            set { _IDO = value; }
        ...
        private void dgvStateControl()
        {
            ...
            /*this.dataGridView1.MergeColumnNames.Add("投入数量");
            this.dataGridView1.MergeColumnNames.Add("完工数量");
            ...
            this.dataGridView1.MergeColumnNames.Add("不良数量");
            this.dataGridView1.MergeColumnNames.Add("良率");
            this.dataGridView1.MergeColumnNames.Add("不良率");*/
            ...
            dataGridView1.Columns["投入数量"].Width = 80;
            dataGridView1.Columns["完工数量"].Width = 80;
            dataGridView1.Columns["复判数量"].Width = 80;
            dataGridView1.Columns["重工数量"].Width = 80;
            dataGridView1.Columns["报废数量"].Width = 80;
            dataGridView1.Columns["不良数量"].Width = 80;
            dataGridView1.Columns["良率"].Width = 80;
            dataGridView1.Columns["不良率"].Width = 80;
        ...
        public void search()
        {
            ...
            sqb.AppendFormat(cbatch_yield .sql);
            sqb.AppendFormat(" WHERE A.WOID LIKE '%{0}%' AND C.CO_WAREID LIKE '%{1}%'", comboBox1.Text, comboBox2.Text);
            sqb.AppendFormat(" AND C.WNAME LIKE '%{0}%' AND A.BATCHID LIKE '%{1}%'", textBox1.Text, textBox2.Text);
            if (checkBox1.Checked)
            {
                sqb.AppendFormat(" AND D.DATE BETWEEN '{0}' AND '{1}'", v1, v2);
               
            }
            sqb.AppendFormat("  GROUP BY SUBSTRING (A.BATCHID,1,15),A.UNIT_LOT ,A.WOID,C.CO_WAREID,C.WNAME,D.Date ");
 ...
 (SELECT USER_GROUP FROM USERINFO WHERE USID='" + LOGIN.USID + "'))" );
            ...
            dt = cbatch_yield.RETURN_HAVE_ID_DT(dt);
            if (dt.Rows.Count > 0)
            {
        ...
        private void BATCH_YIELD_SEARCH_Load(object sender, EventArgs e)
        {
            hint.Text = "";
...
LEFT JOIN WareInfo C ON B.WAREID =C.WareID ");
        ...
        private void btnToExcel_Click(object sender, EventArgs e)
        {
            if (dt.Rows.Count > 0)
            {
                ...
                bc.dgvtoExcel(dataGridView1, "批号良率");

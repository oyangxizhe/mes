...
{
    public partial class STEP_YIELD_SEARCH : Form
    {
        ...
        public STEP_YIELD_SEARCH()
        {
            InitializeComponent();
        }
    
        private void STEP_YIELD_SEARCH_Load(object sender, EventArgs e)
        {
            hint.Text = "";
...
FROM BATCH_DET  A  ");
        ...
        private void dgvStateControl()
        {
            ...
            this.dataGridView1.MergeColumnNames.Add("途程项次");
            ...
            this.dataGridView1.MergeColumnNames.Add("交货日期");
            this.dataGridView1.MergeColumnNames.Add("需求日期");
            this.dataGridView1.MergeColumnNames.Add("下料日期");
            this.dataGridView1.MergeColumnNames.Add("齐套日期");
            this.dataGridView1.MergeColumnNames.Add("建议交期");
            ...
            dataGridView1.Columns["途程项次"].Width = 60;
            ...
            dataGridView1.Columns["交货日期"].Width = 80;
            dataGridView1.Columns["需求日期"].Width = 80;
            dataGridView1.Columns["下料日期"].Width = 80;
            dataGridView1.Columns["齐套日期"].Width = 80;
            dataGridView1.Columns["建议交期"].Width = 80;
        ...
        public void search()
        {
            ...
            sqb.AppendFormat(cbatch.sql);
            sqb.AppendFormat(" WHERE A.WOID LIKE '%{0}%' AND B.CO_WAREID LIKE '%{1}%'", comboBox1.Text, comboBox2.Text);
            sqb.AppendFormat(" AND B.WNAME LIKE '%{0}%' AND F.BATCHID LIKE '%{1}%'", textBox1.Text, textBox2.Text);
            sqb.AppendFormat(" AND G.STEP_ID LIKE '%{0}%' AND G.STEP_ID LIKE '%{1}%'", bc.REMOVE_NAME(comboBox3.Text, '-'), textBox3.Text);
            sqb.AppendFormat(" AND F.ACTION_RULE LIKE '%{0}%'", cposting.RETURN_DB_ACTION_RULE(comboBox4.Text));
            sqb.AppendFormat(" AND F.STATUS LIKE '%{0}%'", cposting.RETURN_DB_STATUS(comboBox5.Text));
            if (checkBox1.Checked)
            {
                sqb.AppendFormat(" AND A.DATE BETWEEN '{0}' AND '{1}'", v1, v2);
               
            }
            sqb.AppendFormat("  ORDER BY A.WOID,G.STEP_ID ASC");
